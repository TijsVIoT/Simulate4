...
#include <HTTPClient.h>  // Zorg ervoor dat je deze header toevoegt
...
// Gebruik de standaard definitie in de bibliotheek
...
const long interval = 1000; // Interval van 1 seconde (1 * 1000 milliseconden)
...
void sendTelegramMessage(const char* message) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;  // Gebruik de HTTPClient klasse
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (Weight < 5.0) {
      ...
      sendTelegramMessage(String(Weight).c_str());
  ...
  delay(10000); // Voeg een korte vertraging toe om de loop te vertragen
}
